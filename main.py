...
def route_list():
	...
	response = requests.post('https://www.minsktrans.by/lookout_yard/Data/RouteList', cookies=cookies,
...
def routes_and_stopes(bus_route):
	data = {
		'p':'minsk',
		'tt':'bus',
		'r': bus_route,
	...
	response = requests.post('https://www.minsktrans.by/lookout_yard/Data/Route', cookies=cookies, headers=headers,
	                         data=data).json()
...
def schedule_for_stop(bus_route, stop, direction):
	data = {
		'p':'minsk',
		'tt':'bus',
		'r': bus_route,
	...
	response = requests.post('https://www.minsktrans.by/lookout_yard/Data/Schedule', cookies=cookies, headers=headers,
	                         data=data).json()
...
if __name__ == '__main__':
	...
	if bus_route not in routes:
		...
		stops_ids = routes_and_stopes(bus_route=bus_route)
		...
		if direction != 0 and direction != 1:
			...
			if stop not in stop_ids_direction:
				...
				schedule_for_stop(bus_route=bus_route, stop=stop, direction=direction)